...
class Read_Clip_list_and_Downloader():
    ...
    def Download_Clips(self):
        ...
        for res_list in self.res_lists:
            ...
            self.Update_CSV(res_list, directory_streamer, directory_date, vid_time, vid_title)

    
    def Update_CSV(self, res_list, directory_streamer, directory_date, vid_time, vid_title):

        res_list['is_downloaded'] = 'O'
        res_list['file_path'] = os.getcwd() +'\\'+ directory_streamer + '\\' + directory_date + '\\['+vid_time+']'+vid_title+'.mp4'
